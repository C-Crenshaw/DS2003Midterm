...
author: "Group 3: Dylan Cole, Carson Crenshaw, Andrew Holzwarth, and Lexi Van Metre"
date: "`r Sys.Date()`"
...
## Data and Dataset Summary
  Our dataset, nbadata.csv, provides individual player and game data from the past three seasons of the NBA (2020, 2021, and 2022 season). In order to conduct this exploratory analysis, data was collected from Advanced Sports Analytics, a major competitor in compiling and analyzing statistical data from the NBA [5]. This study uses the raw metric data from Advanced Sports Analytics which has been validated through comparisons with the public data provided by the NBA online database [6]. While Kaggle or FiveThirtyEight are not used within this report, the dataset is pulled from a reputable source. 
...
* `team_ft_rate`:
Average percentage of free throws attempted in a game. The variable measures a team's ability to get to the free throw line. In other words, the rate represents the percent of total game shots taken at the free throw line. For example, if the given percentage is 0.2, 20% of all shots in a game were attempted free throws, while 80% were attempted field goals. We used the variable as the explanatory variable in the second question.
...
p_load(GGally)
ggcorr(nba_average, title = "Correlation Between Variables", label = TRUE, label_size = 3)
```
...
### Prediction
As a group, we predict the team’s effective field goal percentage will create a positive correlation trend in average team score. In other words, a higher effective field goal percentage may increase alongside a sample mean of points scored.
...
### Discussion
Our prediction was correct. There was a positive trend between effective field goal percentage and average team score. The trend lines between the three individual seasons are almost identical to the overall trend line, which shows little to now variatino over time. When we look at each season individually, we see that the teams that won the most games in the regular season are close to the top right corner of the chart: In 2020 the Milwaukee Bucks went 56-17 (1st in Eastern Conference), and in 2021 the Brooklyn Nets went 48-24 (2nd in Eastern Conference). Interestingly, in the 2022 season the Charlotte Hornets and Minnesota Timberwolves had a high effective field goal percentage and high average team score, but their records were 43-39 and 46-36, respectively. Although the graph shows a good indication that having a high effective field goal percentage leads to more points scored, the trend does not directly translate to winning more games or having success in the postseason. The three teams that won the championship (Los Angeles Lakers, Milwaukee Bucks, and Golden State Warriors) were not the "optimal" points in the chart, which shows that defense or another metric is crucial in determining postseason success.
...
## Question 2:
### Is there a trend between team_ft_rate (tracking fouls), and total score when sorted by team and season?

### Prediction
As a group we predict that a greater number of fouls will increase with the team score. The foul rate predicts the free throw rate percentage of a team: The more fouls committed against a team, the more shots that team takes at the free throw line compared with total game shots. Shooting free throws allows a team to score while the clock is stopped, so it would make sense that a team score is higher if their free throw rate is higher. To reiterate, if the team score is higher, without other confounding variables, we would expect that it would exist alongside a high free throw rate. 
...
### 2.1 Are there any trends when looking across seasons for question 2? 
```{r, echo = FALSE, message = FALSE, warning = FALSE}
...
### Discussion
Our chart shows our prediction is incorrect. There is not a strong trend between free throw rate percentage and average score of a team. While free throws may allow a team to score while the clock is stopped, free throws may affect the team’s offensive rhythm. As we can see from the charts above, the relationship between team score and free throw rate has declined since the 2020 season. This may indicate that over time the teams that are scoring the most points are going to the free throw line less and gaining more points through regulated shots in a game. This can be further justified by the effective field goal percentage in teams. The team’s effective field goals, as shown in chart 1, may be a better predictor of a team’s average score as there was a positive correlation among the past three seasons. 

### Chart Justification
Based on the data, we predicted that a scatter plot would be the best format to showcase our trend and data. Initially, the combined chart showed us an overall predicted trend of all seasons and all teams, sorted by color and shape. Similarly to question 1, we added a trend line to see the overall trend of the data because there was a lot of variance in the data points. To take a closer look, we faceted the data and made three separate charts, one for each season. Within each season, it was easier to compare individual teams and to compare trend lines over time. There was an obvious change in trends that became apparent after faceting the charts. Throughout the past three seasons, team free throw rate is becoming less of a determinant of total points scored. Another potential source of the new trend could be that teams are more focused on shooting three pointers and fouling during a three point shot is less common.

## Conclusion
While the report was founded on some prior basketball knowledge and underlying assumptions, the outcome of the data analysis surprised us. As the data indicated, one can observe a positive trend between the average team effective field goal percentage and the average team score. The conclusion stands when multiple different basketball seasons are considered, with the trend remaining robust over time. 

Despite average free throw rates also demonstrating a positive correlation with average team score, the trend is weak and variable between seasons. A strong, constant relationship between the rate of free throws and team score was not found. Over the past three seasons, free throws have had less of an impact on team score while effective field goal percentage has maintained a strong positive relationship with team score. As a result, team effective field percentage may be a better indicator of team score than team free throw percentage based on the trends. 

This positive relationship between variables, however, does not inherently signal winning more games or having a successful postseason. None of the championship winners from 2020, 2021, or 2022 were the “optimal” data point from the graph. Besides the Milwaukee Bucks in 2021 who were in the top quartile of data points, the 2020 and 2022 championship teams were in the middle of the dataset. The postseason results lead us to believe that there are more important factors, such as defense and team chemistry, that lead to championships, not just offensive prowess. 

Some may argue that a trend between effective field goal percentages and team score seem trivial and obvious, but the graphs illustrate how important it is to consider how low the average effective field goal percentage needs to be to make a difference and to consider the amount of points that represent that difference. Basketball is a high scoring sport with small margins between the winner and the loser. With a new understanding of these non-causal trends between measures of performance and team score across seasons, team managers and executives should glean that small changes in metrics matter. 
